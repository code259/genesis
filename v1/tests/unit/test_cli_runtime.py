...
def test_cli_status_results_and_intervention(tmp_path, monkeypatch):
    ...
    runner = CliRunner()
    run_result = runner.invoke(main, ["run", "--project-id", "cli01", "--spec", str(spec_path), "--max-runs", "2"])
    assert run_result.exit_code == 0, run_result.output

    status_result = runner.invoke(main, ["status", "--project-id", "cli01", "--root", str(project_root)])
    assert status_result.exit_code == 0
    assert '"has_pdf"' in status_result.output

    results_result = runner.invoke(main, ["results", "--project-id", "cli01", "--root", str(project_root)])
    assert results_result.exit_code == 0
    assert '"synthesis_report"' in results_result.output

    intervene_result = runner.invoke(main, ["intervene", "--project-id", "cli01", "--type", "STOP", "--root", str(project_root)])
    assert intervene_result.exit_code == 0


def test_invalid_intervention_type_fails(tmp_path):
    runner = CliRunner()
    result = runner.invoke(main, ["intervene", "--project-id", "cli01", "--type", "INVALID", "--root", str(tmp_path / "projects")])
